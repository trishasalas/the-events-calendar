	...
	class TribeEventsTicketsAttendeesTable extends WP_List_Table {
		...
		function column_check_in( $item ) {
			if ( $item['check_in'] === false ) {
				...
				return sprintf( '<a href="%s"class="button">%s</a>', $item['order_id_url'], __( 'View order', 'tribe-events-calendar' ) );
			}
		...
		function extra_tablenav( $which ) {
			...
			echo sprintf( '<a href="%s" class="export button action">%s</a>', esc_url( add_query_arg( array(
				"attendees_csv"       => true,
